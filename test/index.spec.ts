...
describe('Hello World worker', () => {
	it('root path redirects to GitHub (unit style)', async () => {
		...
		expect(response.headers.get('Location')).toBe('https://github.com/BirdMakingStuff/fxstuff');
	});

	it('root path redirects to GitHub (integration style)', async () => {
		const response = await SELF.fetch(new Request('https://example.com', { redirect: 'manual' }));
		expect(response.status).toBe(302);
		expect(response.headers.get('Location')).toBe('https://github.com/BirdMakingStuff/fxstuff');
	});
